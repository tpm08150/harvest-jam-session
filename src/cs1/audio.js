...
function tick(){
  ...
  /* ⚠️ A SEAM CAN FALL INSIDE A CHORD, and the only check was inside the loop below — which
     runs once per CHORD, not once per bar. A chord here is one, two or four bars long, so a
     row fired on a bar line while a four-bar chord was sounding was not noticed until that
     chord ended: CS·1 held on for up to three bars past the cut, which is the "it keeps
     playing an extra chord when it should have stopped" everybody hears and nobody can
     reproduce, because it depends on where in the progression you happened to press.

     Asked once per TICK as well, against the clock rather than against the schedule, so a
     seam that lands mid-chord is acted on within a tick of arriving. Harmless when the loop
     below has already dealt with it: take() removes what it consumed. */
  Patchwork.scenes.take("cs1", ctx.currentTime);
