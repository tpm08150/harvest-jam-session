...
function renderSteps(el, cfg){
  ...
  /* Anything over 8 splits into two equal rows rather than wrapping wherever the width
     runs out — 16 reads as 2x8, which is also two beats per row. Past 16 it caps at 16
     across, because a 64-step row at this width is a line of slivers. */
  const cols = cfg.steps > 16 ? 16 : (cfg.steps > 8 ? cfg.steps / 2 : cfg.steps);
