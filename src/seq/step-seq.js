...
/* Hold one pad, press another: everything between them becomes one held note. A tie
   ...
   ⚠️ IDEMPOTENT, NOT A TOGGLE, and that is forced rather than chosen. The anchor pad's own
   press has already run by the time the second pad arrives — there is no way to know a
   press is the start of a range until the range happens. So repeating the gesture would
   undo a step the first press had just turned on and leave the ties behind it stranded,
   and what you got back would depend on what the anchor happened to be before you touched
   it. Doing the same thing twice does the same thing; clearing a held note is pressing its
   steps, which is what a press has always meant. */
function tieTo(from, cell){
  ...
  if (!seq.steps[a].on) seq.press(a, o.held ? o.held() : null, {cycle: false});
...
return {
  ...
  down: (cell, vel, mods) => {
    ...
    seq.press(i, o.held ? o.held() : null,
              {accent: !!(mods && mods.accent), cycle: false});
    if (o.repaint) o.repaint();
  }
};
};
