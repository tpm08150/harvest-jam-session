...
const SEQ = {
  ...
  /* THREE WAYS TO WRITE A LINE, because entering one by dragging every step is the slowest
     of them and it was the only one that always worked.

       play     clicks edit the grid — and a step you switch ON takes the note you last
                played, so playing a pitch and tapping four steps writes four of it
       program  a click SELECTS a step, and the next note you play and every knob you move
                belong to that one step. It stays put: you are working ON a step
       step     the same, but the cursor MOVES ON by itself. Play, play, play and the line
                goes in; the arrows walk over steps you want left alone

     Program and Step differ in one line and the difference is the whole point of having
     both: one is for fixing a step, the other is for writing a run of them. */
  mode: "play", sel: 0,
...
function played(midi, vel){
  ...
  if (SEQ.mode === "program" || SEQ.mode === "step"){
    ...
    if (SEQ.mode === "step") selectStep(SEQ.sel + 1);
  }
  return true;
}
...
if (root && Patchwork.onKey) Patchwork.onKey(root, "keydown", e => {
  ...
  if (m === "step" && (e.key === "Backspace" || e.key === "Delete")){
...
el.addEventListener("click", e => {
  ...
  if (seq.SEQ.mode === "program" || seq.SEQ.mode === "step"){
    seq.selectStep(i);
...
el.addEventListener("pointerdown", e => {
  ...
  if (seq.SEQ.mode !== "play") return;             // the click selects; it does not edit
