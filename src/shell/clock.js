...
const TICK_SRC = "\n" + [
  ...
  "  constructor(){ super(); this.n = 0; this.live = true;",
  "    this.port.onmessage = () => { this.live = false; }; }",
  "  process(){",
...
let tickNode = null, tickMute = null, tickModule = null;
...
function ensureTicker(){
  ...
  if (!tickModule){
    ...
    tickModule = ctx.audioWorklet.addModule(url).then(
      () => { URL.revokeObjectURL(url); return true; },
      () => { tickModule = null; return false; });  // stay on the interval
  }
  tickModule.then(ok => {
    /* nothing left to tick for by the time the module is in, or a second start got there first */
    if (!ok || tickNode || running.size === 0) return;
    ...
    tickNode = n; tickMute = mute;
    if (timer != null) setInterval_(BACKSTOP);
  });
}

/* ⚠️ RETIRED WHEN NOTHING RUNS, AND MADE AGAIN ON THE NEXT START. It ticked from the first Play for
   the life of the page: a JavaScript call on the audio thread every quantum, and a message to this
   thread 47 times a second that the handler above threw away with nothing running — on a Raspberry
   Pi 4 that could not spare either (2026-09-13). A processor that returns false from process() is
   finished for good and a node cannot be restarted, so it is told to finish, dropped, and the next
   start builds a fresh node from the module that is already loaded. */
function retireTicker(){
  if (!tickNode) return;
  const n = tickNode, mute = tickMute;
  tickNode = tickMute = null;
  try{ n.port.onmessage = null; n.port.postMessage("stop"); }catch(e){}
  try{ n.disconnect(); mute.disconnect(); }catch(e){}
}
...
return {claim, pin, run, stop, setBpm, onTempo, beatSeconds, setRate, setOriginSource,
        ...
        get bpm(){ return bpm; },
        /* ⚠️ THE TEMPO AS A HUMAN READS IT, which is not the tempo the transport runs on.
           An external clock is measured rather than declared — 24 pulses averaged into
           60000/(perPulse*24) — so it arrives as 119.98765432098765 and every readout that
           printed `bpm` straight out grew ten decimal places and shoved the layout around
           on each pulse. The exact figure is what the scheduler needs and what a jam sends;
           this is what a screen wants, and it lives here because three readouts rounding
           for themselves is three chances to forget. CS·1 and PM·1 already kept the pair
           apart internally and say so — this is the same rule, one level up. */
        get shown(){ return bpm == null ? null : Math.round(bpm); },
