...
/* ⚠️ FUNCTION IS THE MODIFIER. SHIFT IS MOSTLY A SECOND KEYBOARD.
   ...
   ⚠️ TWO THINGS DO READ IT AS STATE, and this note used to say nothing did. layerHeld() takes
   it as a second way into the alt layer, and Shift+Func hops a linked pair — see jump(). Both
   are safe for the same reason: they are built on Shift's own report, which always arrives,
   rather than on some other button changing its number underneath it.

   ⚠️ AND THE HOP IS DELIBERATELY ORDER-FREE, which is not politeness — it is insurance. The
   pair fires on whichever of the two arrives SECOND, so it needs only one of the two presses
   to reach us. If this device turns out to swallow or renumber Func while Shift is held, the
   way it already swallows Shift+pad, then holding Func and tapping Shift still works, because
   Shift's own report is the one thing that always comes through. Neither order was assumed;
   both are wired.

   If it ever seems dead one way round, `Patchwork.surface.traffic` with the thing plugged in
   is the answer, and it is the answer to every question in this file. */
...
function message(io, d, rig){
  ...
  /* Either order: hold one, press the other. The blanket rule at the top of message() has
     already marked Func as used by the time this runs, so its release stays quiet. */
  if (cc === B_SHIFT){
    io.state.shift = v > 0;
    if (v > 0 && io.state.fn) jump(io, rig);
    return;
  }
  ...
  if (cc === B_FUNC){
    if (v > 0){
      io.state.fn = true;
      /* ⚠️ SPENT ON THE PRESS when Shift is already down, or the release would ALSO turn the
      ...
      io.state.fnUsed = !!io.state.shift;
      if (io.state.shift) jump(io, rig);
      return;
    }
    io.state.fn = false;
