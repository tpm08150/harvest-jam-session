...
/* ⚠️ AND CUSTOM 1 IS THE FIFTH SCREEN. The four below are the device's own named modes and
   ...
   ⚠️ THE NUMBER IS A GUESS AND THE OTHERS ARE NOT. Every constant in this file that came from
   the guide rather than from the wire has been wrong at least once — the encoder arrows were
   55/56 on paper and 51/52 in fact. The four below are confirmed against hardware; this one
   is not, because Custom modes are barely documented and there was no device here to ask.

   If Shift + Custom 1 does nothing, one press settles it:
  ...
  if (st === FEAT){
    ...
    if (d[1] === F_ENCS){
      ...
      /* A Custom mode we have not claimed is somebody else's; leave the app where it is
         rather than guessing — but SAY which one, because the only reason Custom 1's number
         is a guess is that there was no hardware here to ask. */
      if (to){ rig.setMode(to.page); rig.goto(to.view); }
      else console.info("launchkey: encoder mode " + d[2] + " is not mapped — "
                      + "see ENC_CUSTOM1 in shell/launchkey.js");
      return;
    }
