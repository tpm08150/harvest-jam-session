...
/* ⚠️ FUNCTION IS THE MODIFIER. SHIFT IS A SECOND KEYBOARD.

   Shift does reach us — it reports on channel 7 as `b6 3f` — but it is not a modifier the
   host can combine with anything, because the device does the combining itself: hold Shift
   and the arrows beside the pads stop sending 106/107 and send 103/102, its own Track pair.
   Shift and a pad go further still and never arrive, being how the device's own pad-mode
   menu is driven.

   So Shift is not read as state and nothing is built on it. What it produces are simply
   OTHER BUTTONS, listed above and bound like any other, and the one modifier this profile
   holds is Func. That reads better anyway — one rule, on one key, meaning the same thing
...
function configureScreen(io){
  ...
  /* And a name for each encoder, so a turn reads "Cutoff 64" instead of "CC21 64". */
  for (let i = 0; i < ENC_N; i++)
    sysex(io, [0x04, SCR_ENC0 + i, CFG_AUTO | ARR_NAME_VALUE]);
...
function message(io, d, rig){
  ...
  /* Func is state, so it needs the release as well as the press. Shift is swallowed here
     without being recorded: it is not a modifier this profile can use, and a flag nothing
...
function paintEncoders(io, rig){
  ...
  for (let i = 0; i < ENC_N; i++){
    ...
    if (s.encs[i] !== v){ s.encs[i] = v; io.send([CH_BTN, ENC_CC0 + i, v]); }

    if (!io.sysex) continue;
    const name = c ? (c.label || c.id || "") : "";
    if (s.encNames[i] === name) continue;
    s.encNames[i] = name;
...
function paintScreen(io, rig){
  ...
  /* ⚠️ THE TITLE CARRIES THE BANK, because the eight names below cannot. On PM·1 they can —
     ...
     kick and the snare. */
  const bankLabel = rig.controlBankName();
