...
/* ⚠️ AND SAY WHAT IT DID. In Custom 1 the per-encoder displays paintEncoders fills never come up for
   ...
   ⚠️ AND THE SCREEN STILL BOUNCES. With this, the re-centre kept to the ends in customTurn() and no auto
   bits on the encoder displays in paintEncoders(), Tyler still saw the flash and the device's readout
   take turns on 2026-09-13. None of the three was enough; HANDOFF.md, "Settings' knobs speak on the MIDI
   port", lists where to look next. */
...
function paintEncoders(io, rig){
  ...
  for (let i = 0; i < ENC_N; i++){
    ...
    /* ⚠️ NO AUTOMATIC READOUT ON CUSTOM 1. The auto bits are what make the device raise these displays
       ...
       Custom 1. Whether a Custom mode's own readout listens to them is not known; if it does, this is what
       stops it. Part of the cache key, so leaving Custom 1 puts them back. */
