...
/* SysEx header. The last byte is the only difference between the full-size and the Mini
   ...
   A universal device inquiry (`f0 7e 7f 06 01 f7`) is answered on both ports. A Mini MK4 25
   says `f0 7e 00 06 02 00 20 29 41 01 00 0N 01 01 09 5c f7`, N being 1 on the DAW port and 0
   on the MIDI port, followed by what reads as a firmware version. Recorded here in case the
   header ever needs choosing rather than doubling — see sysex() — but nothing uses it. */
...
/* Feature controls are queried by sending the same CC on channel 8; the answer comes back
   on channel 7. Guide, "Launchkey MK4 feature controls". Confirmed on the wire: `b7 71 00`
   is answered by `b6 71 NN`, and a plain SET is echoed back in the same shape — which is why
   the first-reply rule in message() is load-bearing and not tidiness. */
const FEAT_ASK = 0xB7;
/* ⚠️ HALF A SECOND, DOWN FROM THE DEVICE'S OWN FOUR OR FIVE. A temporary display is an answer
   ...
   left it unclear whether anything under ten is honoured. Asked back on 2026-09-08 (a Mini
   MK4 25, tools/probe-launchkey.py): set 5, reads back 5; set 0, reads back 0; set 10, reads
   back 10. The number is stored as given and not clamped. Whether the glass then holds a
   half-second display for exactly half a second is still a thing only eyes can check.
...
/* ⚠️ AND CUSTOM 1 IS THE FIFTH SCREEN. The four below are the device's own named modes and
   the app's four performing views; Settings has no place among them and needed somewhere,
   and the pad right of Transport is a mode this profile was previously handing straight back.

   ⚠️ SIX, READ OFF THE WIRE RATHER THAN THE GUIDE — 2026-09-08, tools/probe-launchkey.py
   against a Mini MK4 25. Every constant in this file that came from the guide rather than the
   wire has been wrong at least once (the encoder arrows were 55/56 on paper and 51/52 in
   fact), and this one was a guess until two things settled it. Told to leave DAW mode, the
   device announced the layout it falls back to on its own — `b6 1e 06` — and standalone has
   no Plugin, Mixer, Sends or Transport to fall back to, only Custom. And set-then-ask over
   0-15 found it accepts 1, 2, 4-10 and nothing else: the four named modes, then four in a row
   where four Custom modes would be, then one more (10, unclaimed here; likely the Arp page).
   3 lands on Mixer and 0 on Plugin, so neither exists on this SKU.

   What has NOT been seen is the screen saying "Custom 1" over a `06`, because nobody was at
   the keys. If Shift + Custom 1 ever opens the wrong thing, one press settles it:

     Patchwork.surface.traffic     // press it, then read the tail

   A mode report is `b6 1e NN` on the feature channel. Put NN here. An unrecognised mode is
   also announced on the console rather than dropped in silence, for the same reason.

   ⚠️ AND ITS KNOBS LEAVE THE DAW PORT. Seen 2026-09-13 on the same Mini MK4 25, through a monitor
   bound to its MIDI port: in Custom 1 the encoders send CC 21-28 on channel 1 there, absolute, from
...
/* What Settings lends the knobs while Shift is up, and how long an answer to that counts as ours —
   ...
   thread, which is drawing Settings and its card at that very moment; fed 400 ms late in the harness,
   an answer was followed as a press and the app went to Studio. A Shift change ends the wait sooner —
   see setShift() — so this only has to outlast a stall. */
const ENC_LENT = ENC_PLUGIN;
const LAYOUT_ECHO_MS = 1500;
...
/* The Launchpad palette, by the indices worth naming. Hues are four apart because each one
   has FOUR shades — n is full brightness and n+1 through n+3 step down from it — which is
   what makes "this pad exists" and "this pad is doing something" the same colour at two
   levels rather than two colours the eye has to learn. Guide, "Colour palette". */
...
/* ⚠️ HOW FAR DOWN "NOT DOING ANYTHING" SITS, and the one number to change if the answer
   looks wrong on the hardware. It was n+2, the middle of the three dark shades, and against
   a launcher whose columns are now seven different colours that was not far enough down —
   a grid where every pad is lit in some colour makes the four that hold a pattern hard to
   pick out, which is the one thing the grid is for.

   n+3 is the dimmest the palette offers before off, so a pad that exists still says so and
   a pad with something in it is unmistakably brighter. Raise it back to 2 if a dark room
   makes the empty ones vanish; 4 would be off, which is a different statement — an empty
   cell you can still press must not look like a pad that does nothing. */
const DIM_STEP = 3;
/* ⚠️ White is the exception and cannot follow the rule: it lives at 3 with the greys BELOW
   it — 1 dark, 2 mid, 3 full — so its dark version is found by going down rather than up,
   and 1 is already as far as it goes. */
const dim = c => (c && c !== PAL.white) ? c + DIM_STEP : (c === PAL.white ? 1 : 0);
...
function detect(inputs, outputs){
  ...
  const keysOut = outs.find(p => p.id !== ctrlOut.id) || null;

  return {
    ctrlIn: ctrlIn.id,
    ctrlOut: ctrlOut.id,
    keysIn: keysIn ? keysIn.id : null,
    keysOut: keysOut ? keysOut.id : null,
...
function start(io, rig){
  io.state = {
    ...
    /* The device's own encoder layout, as it last reported it, and for Custom 1 — see keys() —
       where each knob's counter last was, whether it has been asked back to the middle since, and
       the channel it speaks on. */
    encLayout: ENC_PLUGIN,
    lent: false, layoutSet: null, layoutSetAt: 0,   // Settings' borrowed layout — see lendKnobs()
    customAt: new Array(ENC_N).fill(null),
    customParked: new Array(ENC_N).fill(false),
    customCh: 0,
    customSeen: false,                 // a Custom 1 knob has been heard on the MIDI port
    centres: null, centreVotes: 0,     // whether a Custom 1 knob takes a re-centre — see customTurn()
    turnFlashAt: 0, turnFlashTimer: null,
    ...
    down: [],
    pressedOn: new Map(),              // pad -> the grid its press went to, for its release
...
function stop(io){
  if (io.state && io.state.turnFlashTimer){ clearTimeout(io.state.turnFlashTimer); io.state.turnFlashTimer = null; }
...
/* ---- the knobs in Custom 1 ----
   ⚠️ THEY ARRIVE ON THE MIDI PORT, AS A COUNTER. Read off a Mini MK4 25 on 2026-09-13: in Custom 1
   the encoders send CC 21-28 on channel 1 of the port handed to the router, as absolute values from
   the device's own counter — which starts at 0, stops at 0 and 127, and took no notice of the
   positions this profile parks DAW-mode knobs at. So on Settings, the one page on a Custom mode, the
   pads worked and the knobs went to whichever panel had the focus as ordinary control changes.

   So they are the surface's while the device says it is on Custom 1: ownsKeys() keeps them from the
   instruments, keys() turns them, and a list still moves one detent, one position. The first report
   from each knob is only a baseline. */
function ownsKeys(io, d){
  const s = io.state;
  return !!(s && d && d.length >= 3 && s.encLayout === ENC_CUSTOM1
            && (d[0] & 0xF0) === 0xB0 && d[1] >= ENC_CC0 && d[1] < ENC_CC0 + ENC_N);
}
function keys(io, d, rig){
  if (!ownsKeys(io, d)) return false;
  const s = io.state;
  /* a turn while Func or ">" is down spends it, exactly as a turn on the DAW port does */
  if (s.fn) s.fnUsed = true;
  if (s.act) s.actUsed = true;
  s.customSeen = true;
  s.customCh = d[0] & 0x0F;
  customTurn(io, rig, d[1] - ENC_CC0, d[2]);
  return true;
}

/* ⚠️ WHICH WAY IT TURNED, WITHOUT KNOWING WHETHER THE MIDDLE STUCK. A counter at 0 cannot be turned
   lower, so a knob nearing either end is asked back to 64 on the port it spoke on — and whether a
   Custom mode honours that is not known. The first cut decided it afresh from every report, and got
   it wrong whenever the device's answer and the hand's next detent crossed on the wire: a fast turn
   read as no turn, which is the glitch. So it is LEARNED, from reports only one answer explains — a
   report near 64 from a knob that had been far from it means the middle stuck, a report beside the
   last one and far from 64 means it did not — and two agreeing settle it. A device that does not
   honour it is not asked again. Until it is settled each report is read from whichever baseline it is
   nearer, never from one it has not moved from; and exactly 64 straight after asking is the device
   repeating us, not a turn. */
function customTurn(io, rig, i, v){
  const s = io.state;
  const c = rig.controls(layerHeld(io))[i];
  const prev = s.customAt[i], parked = s.customParked[i];
  s.customAt[i] = v;
  s.customParked[i] = false;
  if (!c) return;
  if (typeof c.nudge !== "function"){
    if (typeof c.set === "function"){ try{ c.set(v / 127); }catch(e){} }
    showTurn(io, c);
    return;
  }
  if (parked && v === ENC_MID) return;
  if (prev != null){
    const fromLast = v - prev, fromMid = v - ENC_MID;
    let d = fromLast;
    if (parked && s.centres === true) d = Math.abs(fromMid) <= 6 ? fromMid : fromLast;
    else if (parked && s.centres == null){
      if (Math.abs(prev - ENC_MID) > 8){
        if (Math.abs(fromMid) <= 3) s.centreVotes++;
        else if (Math.abs(fromLast) <= 3) s.centreVotes--;
        if (s.centreVotes >= 2) s.centres = true;
        else if (s.centreVotes <= -2) s.centres = false;
      }
      d = !fromLast ? fromMid : !fromMid ? fromLast
        : (Math.abs(fromMid) < Math.abs(fromLast) ? fromMid : fromLast);
    }
    if (!d) return;
    try{ c.nudge(d > 0 ? 1 : -1); }catch(e){}
  }
  /* ⚠️ ONLY NEAR THE ENDS, AND BEFORE THE SCREEN. On 2026-09-13 Tyler saw the screen bounce between the
     flash below and the device's own generic readout for a Custom 1 knob. A re-centre is a control change
     sent to that knob, and if the device answers one the way it answers a turn — not known — a re-centre
     after every turn put its readout back over the flash every time. So it goes only when the counter is
     within CUSTOM_EDGE of an end, the one time it is needed, and ahead of the flash, so the flash is drawn
     last either way. */
  if (s.centres !== false && (v <= CUSTOM_EDGE || v >= 127 - CUSTOM_EDGE)) parkCustom(io, i);
  showTurn(io, c);
}
const CUSTOM_EDGE = 12;
function parkCustom(io, i){
  io.state.customParked[i] = true;
  if (io.sendKeys) io.sendKeys([0xB0 | io.state.customCh, ENC_CC0 + i, ENC_MID]);
}

/* ⚠️ AND SAY WHAT IT DID. In Custom 1 the per-encoder displays paintEncoders fills never come up for
   these knobs — the device puts up a generic readout of its own instead — so Settings changed with
   nothing useful on the screen. The global temporary display, as for a button.

   ⚠️ AT ONCE, NOT PACED. It was held to one message every 50 ms while a knob spun. The device draws its
   own readout when a knob moves, so for as long as the pacing waited that readout could be all the
   screen showed — and Tyler saw the two take turns. Reports closer together than 20 ms, about a frame,
   are still folded into one, and the last always shows the value the knob finished on.

   ⚠️ AND THE SCREEN STILL BOUNCED. With this, the re-centre kept to the ends in customTurn() and no auto
   bits on the encoder displays in paintEncoders(), Tyler still saw the flash and the device's readout
   take turns on 2026-09-13. None of the three was enough. What stopped it was not being in Custom 1
   while the knobs turn — see lendKnobs() — so this now only answers a knob turned with Shift held. */
function showTurn(io, c){
  const s = io.state;
  if (s.turnFlashTimer) return;               // one is already on its way, and will read the latest
  const send = () => {
    s.turnFlashTimer = null;
    s.turnFlashAt = performance.now();
    let text = "";
    try{
      if (typeof c.text === "function") text = String(c.text() == null ? "" : c.text());
      else if (typeof c.get === "function") text = Math.round(c.get() * 100) + "%";
    }catch(e){}
    flash(io, c.label || c.id || "", text);
  };
  const wait = 20 - (performance.now() - (s.turnFlashAt || 0));
  if (wait <= 0) send();
  else s.turnFlashTimer = setTimeout(send, wait);
}

/* ---- Settings lends its knobs a DAW layout ----
   ⚠️ CUSTOM 1 IS FOR THE MODE PADS, NOT FOR THE KNOBS. A knob turned in a Custom mode is the device's
   own: it puts up a generic readout of the control change as the knob moves, and on 2026-09-13
   nothing the page sent kept that off the screen — not a flash within 20 ms of every report, not
   re-centres kept to the ends, not the auto bits taken off the encoder displays. Tyler saw the flash
   and the readout take turns through all three. No other page has shown that readout: their knobs are
   on DAW layouts, where the displays are the app's to raise.

   And Custom 1 only has to be the layout while Shift is down. It is how the device says "Settings",
   and what the mode pads under Shift light and change from; once Shift is up, nothing reads the
   layout but the knobs. So Settings asks for Plug-in when Shift comes up — the knobs then speak on
   the DAW port and move Settings through the same list code as every other page, under their own
   displays — and asks for Custom 1 back the moment Shift goes down, so any mode pad pressed is still
   a change the device reports, Plug-in included, and Custom 1 is what is lit.

   The device echoes a layout it is sent (tools/probe-launchkey.py, 2026-09-08); the F_ENCS report
   in message() notes an echo of our own request instead of following it. One request at a time. A
   lost echo leaves the knobs where they were, and the next Shift asks again.

   Tried on Tyler's Mini MK4 25 on 2026-09-13: lent, the knobs worked and the screen stopped bouncing.
   Not reported: whether a sent layout puts a mode name on the screen. If Settings' knobs ever go dead
   once Shift is up, the lend did not take. */
function lendKnobs(io, rig){
  const s = io.state;
  if (!s || rig.mode !== ENC_VIEW[ENC_CUSTOM1].page) return;
  if (s.layoutSet != null && performance.now() - s.layoutSetAt < LAYOUT_ECHO_MS) return;
  let want = null;
  if (!s.shift && s.encLayout === ENC_CUSTOM1) want = ENC_LENT;
  else if (s.shift && s.lent && s.encLayout === ENC_LENT) want = ENC_CUSTOM1;
  if (want == null) return;
  s.layoutSet = want;
  s.layoutSetAt = performance.now();
  io.send([FEAT, F_ENCS, want]);
}
/* Shift arrives by two roads; both end here, and only a change does anything. ⚠️ A CHANGE ALSO ENDS
   ANY WAIT FOR AN ANSWER. The device answers a request before it reports anything the hand does after
   it, so an answer not in by the time Shift moves is not coming, and a layout reported from here on is
   the hand's choice — the only way a mode pad is reached at all is with Shift down. */
function setShift(io, rig, down){
  if (!io.state || io.state.shift === down) return;
  io.state.shift = down;
  io.state.layoutSet = null;
  lendKnobs(io, rig);
}
/* ⚠️ THE KNOBS CHANGE PORTS UNDER THE SAME CONTROLS, so paintEncoders() would not notice: nothing
   arrived, so no list is parked mid-travel and no baseline set, and the first detent on the DAW port
   would be read against wherever the knob was last left in Plug-in. Every knob is made to arrive
   again, and painted now rather than on a tick a turn could beat. */
function reseat(io, rig){
  io.state.encIds.fill("\u0000");
...
function message(io, d, rig){
  ...
  if (st === FEAT){
    ...
    if (d[1] === F_ENCS){
      const s = io.state;
      /* ⚠️ AN ANSWER TO OUR OWN REQUEST IS NOT A PAD PRESS. Settings lends the knobs a layout — see
         lendKnobs() — and the device echoes what it is sent. Followed like a press, lending Plug-in
         would take the app straight to Studio. Noted instead, and the lending carries on from there. */
      const ours = s.layoutSet === d[2] && performance.now() - s.layoutSetAt < LAYOUT_ECHO_MS;
      s.encLayout = d[2];
      if (ours){
        s.layoutSet = null;
        s.lent = d[2] === ENC_LENT;
        if (s.lent) reseat(io, rig);
        lendKnobs(io, rig);               // Shift may have moved while the answer was on its way
        return;
      }
      s.lent = false;
    ...
    if (d[1] === F_SHIFT){ setShift(io, rig, d[2] > 0); return; }
  ...
  if (type === 0x90 || type === 0x80){
    ...
    /* ⚠️ A RELEASE GOES TO THE GRID THAT HAD THE PRESS, not to whichever grid is up when the
       finger lifts. A press and its release are one gesture: the punch page holds an effect in
       until its pad comes up, so leaving the page with a finger still down — Func, or Shift and
       a mode pad — handed the release to a grid with nothing to let go of, and the effect stayed
       in with no pad left that knew it was holding it. CS·1's chord pads had the same hole across
       a change of focus. */
    const pressed = io.state.pressedOn;
    const g = on || !pressed.has(i) ? rig.grid() : pressed.get(i);
    ...
    if (on){ if (held.indexOf(i) < 0) held.push(i); pressed.set(i, g); }
    else { const k = held.indexOf(i); if (k >= 0) held.splice(k, 1); pressed.delete(i); }
    if (!g) return;
    try{
  ...
  if (cc >= ENC_CC0 && cc < ENC_CC0 + ENC_N){
    /* ⚠️ NOT WHILE THE MIDI PORT HAS THE KNOBS. On Custom 1 they speak there — see keys() — and a copy
       arriving here as well would be read against a parked middle the device never took, and pull the
       other way. Only once the MIDI port has actually been heard from, so a device that keeps Custom 1
       on this port still works. */
    if (io.state.encLayout === ENC_CUSTOM1 && io.state.customSeen) return;
  ...
  if (cc === B_SHIFT){ setShift(io, rig, v > 0); return; }
...
function paintEncoders(io, rig){
  ...
  for (let i = 0; i < ENC_N; i++){
    ...
    /* ⚠️ NO AUTOMATIC READOUT ON CUSTOM 1. The auto bits are what make the device raise these displays
       itself when a knob moves, which is right for a DAW-mode turn and competes with showTurn()'s flash on
       Custom 1. Taking them off did not stop the device's own Custom-mode readout on 2026-09-13, so that is
       not raised through them; kept because it costs nothing, while Settings now lends the knobs a DAW
       layout instead — see lendKnobs(). Part of the cache key, so leaving Custom 1 puts them back. */
    const auto = s.encLayout === ENC_CUSTOM1 ? 0 : CFG_AUTO;
    const mode = !c ? (auto ? "" : "x") : (wantsText ? "t" : "n") + (auto ? "a" : "");
    if (s.encMode[i] !== mode){
      s.encMode[i] = mode;
      s.encText[i] = "\u0000";                   // force the value to be written afresh
      sysex(io, [0x04, SCR_ENC0 + i,
                 auto | (wantsText ? ARR_2LINE : ARR_NAME_VALUE)]);
    }
