...
const rig = {
  ...
  controls(layer){
    ...
    if (key && typeof f.spec[key] === "function"){
      try{ list = (f.spec[key]() || []).slice(0, 8); }catch(e){ list = []; }
    }
...
/* ---- which of an instrument's sixteen sequences is up, as a control ----
   ⚠️ FUNC AND THE EIGHTH ENCODER, ON EVERY INSTRUMENT THAT KEEPS SEQUENCES. The strip was the only
   way to choose one, and it is on the screen rather than under the hands, so writing a song from
   the controller meant reaching for the mouse between parts.

   A list like any other on the encoders: one detent, one sequence, and it stops at 1 and 16 rather
   than wrapping, because overshooting from 16 to 1 on a running instrument would jump to a
   different part entirely. Choosing is sequences.select(), which is exactly what a click on the
   strip does — an empty sequence comes up blank, and walking through one leaves it empty. From a
   grid no sequence holds (the strip's ring out) the first detent goes to 1.

   ⚠️ IT SELECTS ON EVERY DETENT, as the strip does on every click. Spun quickly past several
   sequences on a running instrument, a step of one in between can sound on its way past. */
function sequence(id){
  const Q = window.Patchwork && Patchwork.sequences;
  if (!id || !Q || !Q.has(id)) return null;
  const at = () => Q.at(id);
  const go = i => { if (i !== at()) Q.select(id, i); };
  return {
    id: "sequence-" + id, label: Q.label(id), short: "Seq", stepped: true,
    text: () => (at() < 0 ? "none" : String(at() + 1)),
    get: () => Math.max(0, at()) / (Q.COUNT - 1),
    set: v => go(Math.round(Math.max(0, Math.min(1, v)) * (Q.COUNT - 1))),
