...
/* ---- audio time into port time ----
   ...
   ⚠️ CS·1 AND PM·1 USE IT TOO, although they send through ports of their own. Their perfTime()
   asked getOutputTimestamp(), which stamps a note for when its audio frame leaves the device
   rather than when it is rendered, and so put their notes 12–15 ms after everyone else's on
   the same cable. Every scheduled note is now stamped the same way, whichever instrument sends
   it; DR·1's sendHit() does the same sum inline. See HANDOFF.md, "SQ·1 asks for a row from
   every track", and ctxPerfBase() in cs1/midi.js for the one place the latency pairing is
   still right. */
function portTime(t){
